...
import com.example.imu_demo.data.IMUResult
import com.example.imu_demo.data.IMUReceiveManager
...
class IMUBLEReceiveManager @Inject constructor(
    ...
    override val data: MutableSharedFlow<Resource<IMUResult>> = MutableSharedFlow()
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if(status == BluetoothGatt.GATT_SUCCESS){
                if(newState == BluetoothProfile.STATE_CONNECTED){
                    ...
                    this@IMUBLEReceiveManager.gatt = gatt
                } else if(newState == BluetoothProfile.STATE_DISCONNECTED){
                    coroutineScope.launch {
                        data.emit(Resource.Success(data = IMUResult(0f,0f,ConnectionState.Disconnected)))
                    }
                    gatt.close()
                }
            }else{
            ...
            with(characteristic){
                when(uuid){
                    UUID.fromString(TEMP_HUMIDITY_CHARACTERISTICS_UUID) -> {
                        ...
                        val tempHumidityResult = IMUResult(
